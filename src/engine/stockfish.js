...
function getEngine() {
  return new Promise((resolve, reject) => {
    if (_engineReady && _engine) { resolve(_engine); return; }
    ...
    _engine.onmessage = (event) => {
      ...
      if (!msg) return;
      if (msg.includes('uciok'))    { _engine.postMessage('isready'); }
  ...
  return new Promise((resolve, reject) => {
    ...
    engine.onmessage = (event) => {
      ...
      if (msg.startsWith('info') && msg.includes('score')) {
        ...
        if (parsed) { lines[parsed.multiPV] = parsed; if (onInfo) onInfo(parsed); }
      }
...
export async function getBestMove(fen, depth = 12) {
  const engine = await getEngine();
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error('getBestMove timed out')), 12000);
    engine.onmessage = (event) => {
...
export async function analyzeGame(moves, options = {}) {
  ...
  for (let i = 0; i < moves.length; i++) {
    try {
      const fenBefore = fen;   // ← capture FEN before the move
      ...
      results.push({
        ...
        fen:            fenBefore,   // ← FEN at the moment of the mistake
      });
    } catch (e) {
...
function parseInfoLine(line) {
  ...
  const get      = (key) => { const idx = tokens.indexOf(key); return idx !== -1 ? tokens[idx + 1] : null; };
  const depthVal   = parseInt(get('depth'))   || 0;
  const multiPVVal = parseInt(get('multipv')) || 1;
  const scoreType  = tokens.includes('cp') ? 'cp' : tokens.includes('mate') ? 'mate' : null;
  ...
  return {
    ...
    pv,
    evalString: scoreType === 'mate' ? `M${Math.abs(scoreVal)}` : formatCP(scoreVal),
  };
}
