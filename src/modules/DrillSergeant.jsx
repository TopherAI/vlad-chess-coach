 ...
 *
 * Takes blunder/mistake positions from GameAutopsy and turns them into
 * targeted interactive puzzles. Drills from YOUR games, not generic puzzles.
 *
...
const DRILL_SOURCES = {
  AUTOPSY:  "autopsy",   // Loaded from GameAutopsy blunder positions
  MANUAL:   "manual",    // FEN pasted manually
  WEAKNESS: "weakness",  // Loaded from coachMemory weakness fingerprint
};

const WEAKNESS_TAGS = [
  "Move 9 Speed Trap",
  "Panic Simplification",
  "Queen Sortie Response",
...
// ---------------------------------------------------------------------------
// Drill builder — creates drill objects from GameAutopsy worstMoves
// ---------------------------------------------------------------------------

/**
 * Convert GameAutopsy worstMoves array into drill queue.
 * @param {object[]} worstMoves  — from gameAnalysis.summary.worstMoves
 * @param {string[]} fenSequence — FEN at each move index (pass from GameAutopsy)
 * @returns {Drill[]}
 */
export function buildDrillsFromAutopsy(worstMoves, fenSequence) {
  return worstMoves
    .filter(m => m.bestMove && fenSequence[m.moveIndex])
    .map((m, i) => ({
      id:             `autopsy-${i}`,
      source:         DRILL_SOURCES.AUTOPSY,
      fen:            fenSequence[m.moveIndex],
      ...
      side:           m.side,
      weaknessTag:    inferWeaknessTag(m),
      attempted:      false,
      solved:         false,
    }));
}

/** Map a blunder pattern to a weakness tag for the memory fingerprint */
function inferWeaknessTag(move) {
  if (move.moveNumber >= 8 && move.moveNumber <= 12) return "Move 9 Speed Trap";
  if (move.cpLoss > 200) return "Missing Forcing Moves";
  return null;
}

// ---------------------------------------------------------------------------
// Chessboard renderer (uses chessboard.js via CDN)
// ---------------------------------------------------------------------------
...
function ChessBoard({ fen, orientation, onMove, highlightSquares = [], disabled = false }) {
  ...
  // Overlay highlight squares
  const squareSize = boardRef.current
    ? boardRef.current.offsetWidth / 8
    : 60;
  ...
  return (
    ...
    >
      ...
      <div style={styles.queueInfo}>
        ...
        {drill.weaknessTag && (
          <span style={styles.queueTag}>{drill.weaknessTag}</span>
        )}
      </div>
...
/** Progress bar across drill queue */
...
export default function DrillSergeant({ initialDrills = [] }) {
  ...
  const [feedback, setFeedback]       = useState(null);   // null | 'correct' | 'close' | 'wrong' | 'revealed'
  ...
  const fireVladFeedback = useCallback(async (drill, solved, attemptsCount) => {
    ...
    try {
      const context = `
Drill position — Move ${drill.moveNumber} (${drill.side})
Original classification: ${drill.classification} (−${drill.cpLoss}cp)
Solution move: ${drill.solutionMove}
Player's actual game move: ${drill.playerMove ?? "unknown"}
Weakness tag: ${drill.weaknessTag ?? "general"}
Result: ${solved ? `SOLVED in ${attemptsCount} attempt(s)` : "Required hint/reveal"}
Give a single sharp sentence of coaching feedback. No fluff. Be Vlad.
      `.trim();
            ...
            <>
              ...
              <div style={styles.drillHeader}>
                <div style={styles.drillMeta}>
                  ...
                  <span style={styles.drillDesc}>
                    Move {activeDrill.moveNumber} · {activeDrill.side} to move
                  </span>
              ...
              {/* 4-Step reminder (Vlad's protocol) */}
