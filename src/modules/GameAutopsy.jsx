...
function MoveRow({ move, isSelected, onClick, sanMove }) {
  ...
  const deviated = sanMove
    ? isGmDeviation(move.moveNumber, move.side, sanMove)
    : false;
    ...
    >
      ...
      {deviated && (
        <span
          ...
          title={`GM Plan: ${ITALIAN_CAGE_PLAN[`${move.moveNumber}${move.side === "white" ? "w" : "b"}`]}`}
        >
          ⚠️ Off-Plan
        </span>
      )}
        ...
        <div style={styles.results}>
          ...
          <div style={styles.tabs}>
            {TABS.map(tab => (
              ...
              >
                {tab.label(criticalMoves.length)}
              </button>
            ))}
          </div>
              ...
              <div style={styles.criticalToolbar}>
                <span style={styles.criticalInfo}>
                  {criticalMoves.length > 0
                    ? `${criticalMoves.length} critical move${criticalMoves.length > 1 ? "s" : ""} · saved locally`
                    : "No critical moves this game"}
                </span>
                {criticalMoves.length > 0 && (
            ...
            <div style={styles.moveList}>
              ...
              {analysis.moves.map((move, i) => (
                <MoveRow
                  ...
                  sanMove={gameInfo?.sanMoves?.[i] ?? null}
                />
              ))}
            </div>
          )}
